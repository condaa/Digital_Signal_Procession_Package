...
namespace DSPAlgorithms.Algorithms
{
    public class FIR : Algorithm
    {
        ...
        public override void Run()
        {
            ...
            int N = get_N(InputStopBandAttenuation, delta_f, InputFS);

            double hd;
            double wc;

            Signal signal = new Signal(new List<float>(), false, new List<float>(), new List<float>(), new List<float>());
            signal.Samples = Enumerable.Repeat(default(float), N).ToList();
            signal.SamplesIndices = Enumerable.Repeat(default(int), N).ToList();
            ...
            if (InputFilterType == FILTER_TYPES.LOW)
            {
                ...
                for (int n = 0; n <= N / 2; n++)
                {
                    ...
                    signal.Samples[N / 2 - n] = (float)(hd * wc);
                    signal.Samples[N / 2 + n] = (float)(hd * wc);

                    signal.SamplesIndices[N / 2 - n] = -n;
                    signal.SamplesIndices[N / 2 + n] = n;
                }
            ...
            OutputHn = new Signal(new List<float>(signal.Samples), new List<int>(signal.SamplesIndices), false);

            DirectConvolution convolution = new DirectConvolution();
            convolution.InputSignal1 = InputTimeDomainSignal;
            convolution.InputSignal2 = signal;
            convolution.Run();
